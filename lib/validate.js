...
export function lintTasks (files) {
  ...
  for (const { name, tasks } of files) {
    ...
    for (const [i, t] of tasks.entries()) {
      ...
      if (!isNil(t['deps']) && !typesafeIsArray(t['deps'])) {
  ...
  for (const { name, tasks } of files) {
    ...
    for (const t of tasks) {
      ...
      for (const field of REQUIRED_FIELDS) {
        if (isNil(t[field]) || t[field] === '') {
          err(name, `task ${label} missing required field: ${field}`);
        }
      }

      if (!isNil(t['id'])) {
        // ONE answer to "what is this row's id", used by every line below it. `ID_RE.test`
        // already coerced, and `nsId` — the loader's namespacing, the thing that decides what
        // a row is actually CALLED — is `String(ref)` too. The duplicate check was the only
        // line still comparing raw values, so `id: 1` and `id: '1'` in one file were two ids
        // here and ONE `GlobalId` in the loader: the second row silently shadows the first in
        // `ready.js`'s `byId` map, and validate exits 0 saying the file is fine.
        //
        // This closes the WITHIN-FILE, type-mismatched case only. Two rows carrying the same
        // id as the same type already errored, and the loader owing a warning when two rows
        // collapse into one `GlobalId` is `diarie-did`.
        const id = String(t['id']);
        if (!ID_RE.test(id)) {
          err(name, `task ${label}: invalid id (expected ${ID_RE.source})`);
        }
        if (seen.has(id)) {
      ...
      if (!isNil(t['status']) && !isStatus(t['status'])) {
        err(name, `task ${label}: invalid status "${t['status']}"`);
      }
      if (!isNil(t['type']) && !isTaskType(t['type'])) {
        err(name, `task ${label}: invalid type "${t['type']}"`);
      }
      if (!isNil(t['priority']) && !isPriority(t['priority'])) {
        err(name, `task ${label}: invalid priority "${t['priority']}"`);
      }
      if (!isNil(t['updated']) && (!isType(t['updated'], 'string') || !Number.isFinite(Date.parse(t['updated'])))) {
        ...
        err(name, `task ${label}: invalid updated ${renderRejected(t['updated'])} (expected an ISO date string)`);
      }
      ...
      for (const field of STRING_FIELDS) {
        if (!isNil(t[field]) && !isType(t[field], 'string')) {
          err(name, `task ${label}: invalid ${field} ${renderRejected(t[field])} (expected a string)`);
        }
      }
      if (!isNil(t['id'])) {
