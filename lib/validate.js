...
function lintFields (row, noun, err) {
  ...
  //
  // The caller's duplicate check asks the OTHER question — "what does this row CLAIM to be
  // called" — and its `String()` stays, because it is the only thing that can still see the
  // clash at all. The loader no longer collapses `42` onto `'42'`: it refuses the numeric row
  // outright, so the two never meet in `byId`. What remains is an authoring error the file
  // still contains — one name claimed twice — and only a coercing comparison notices it.
  // Verified: such a file reports BOTH, `invalid id 42` and `duplicate id "42" within file`.
  ...
  for (const { name, tasks } of files) {
    ...
    for (const t of tasks) {
      ...
      if (!isNil(t['id'])) {
        ...
        // warning when two rows collapse into one `GlobalId` is `diarie-did` — which is now
        // reachable only for two rows claiming the SAME LITERAL id, since a type-mismatched
        // pair no longer meets in the loader at all (see `lintFields`' id check).
