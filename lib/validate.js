 ...
 * IO and owns the exit codes; `store.js` owns finding the store.
 *
...
function lintFields (row, noun, err) {
  ...
  // `String(row['id'])` is the ONE answer to "what is this called", shared with the caller's
  // duplicate check below. `ID_RE.test` coerces anyway, and `nsId` — the loader's namespacing,
  // the thing that decides what a row is actually CALLED — is `String(ref)` too. The duplicate
  // check was the only line still comparing raw values, so `id: 1` and `id: '1'` in one file
  // were two ids here and ONE `GlobalId` in the loader: the second row silently shadows the
  // first in `ready.js`'s `byId` map, and validate exits 0 saying the file is fine.
  if (!isNil(row['id']) && !ID_RE.test(String(row['id']))) {
    err(`${noun} ${label}: invalid id (expected ${ID_RE.source})`);
  }
