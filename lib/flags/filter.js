...
export function validateFilterFlags ({ filter }) {
  ...
  if (!isStatus(filter)) {
    // NAME WHAT WAS REJECTED, matching `--days`'s `(got ...)` idiom one file over. Listing the
    // alternatives says what is allowed; it does not say what the tool actually received, so a
    // refusal that omits the value cannot be told apart from one that quietly dropped it. That
    // is the same distinction the MISTAKES table now asserts for every user error — and its
    // absence is how an unknown top-level flag came to be filed as "not a mistake" at all.
    throw new InputError(`--filter must be one of: ${[...VALID_STATUSES].join(', ')} (got ${JSON.stringify(filter)})`, undefined, 'EUSAGE');
  }
