...
import {
  isNil, isPriority, isStatus, isTaskType, LEGACY_TRACKER_DIRS, nsId, TRACKER_DIRS, TRACKER_LABEL,
} from './schema.js';
  ...
  constructor (from, searched, legacy) {
    super(legacy
      ...
      ? `found ${legacy} — the store is now ${TRACKER_LABEL}. Rename it: ` +
        `\`git mv ${legacy} ${TRACKER_DIRS[0]}\` (or \`${TRACKER_DIRS[1]}\` to keep it dotted)`
      : (searched
 ...
 */
export class TwoStoresError extends Error {
  /** @override */
  name = 'TwoStoresError';
 ...
 *
 * @param {string} dir
 * @returns {string | undefined}  the legacy path, absolute
 */
function legacyTrackerDirIn (dir) {
  return LEGACY_TRACKER_DIRS.map(name => join(dir, name)).find(p => existsSync(p));
}
...
export function resolveRoot ({ cwd = process.cwd(), root } = {}) {
  ...
  if (explicit) {
    ...
    if (!trackerDirIn(dir)) throw new NoStoreError(dir, false, legacyTrackerDirIn(dir));
    return dir;
  }
  ...
  for (;;) {
    ...
    if (legacy) throw new NoStoreError(dir, true, legacy);
...
export async function listTaskFiles (root) {
  ...
  if (!store) throw new NoStoreError(resolve(root), false, legacyTrackerDirIn(root));
