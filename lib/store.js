 ...
 *
 * `existsSync` was the bug: it answers "is there anything here", and a store is not anything
 * — it is a directory. A plain file named `diarium` (a stray note, a shell redirect that lost
 * its argument, a `.gitignore`d artifact) answered yes, so the store "resolved", `readdir` on
 * its `tasks/` failed the not-found way, and the reader printed `{"ready":[]}` and exited 0.
 * A confident empty backlog served from a non-store: the founding defect, reached through a
 * filesystem detail rather than a missing store. The visible form of the pair collides far
 * more readily than the dotted one ever did, which is what turned this from pedantry into a
 * defect the day the pair shipped.
 *
...
export async function listTaskFiles (root) {
  ...
  if (!store) throw new NoStoreError(resolve(root), false, legacyTrackerDirIn(root)?.path);
