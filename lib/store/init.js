...
// `node:path`, NEVER `node:path/posix`. The posix variant does not treat a Windows drive
// letter as absolute, so `resolve('C:\\Users\\me\\proj')` returns `<cwd>/C:\Users\me\proj`
// — `init` would create the store at a fabricated path, and every reader (which resolves
// via `node:path`) would then answer ENOSTORE for a store that exists. CI is ubuntu-only,
// so nothing here can go red; the `no-platform-path-import` ast-grep rule is the guard.
import { join, resolve } from 'node:path';
import { InputError } from '../utils/errors.js';
import { legacyTrackerDirIn, rootFromEnv, trackerDirIn } from './root.js';
import { isDirectory } from './utils.js';
import {
  defaultTrackerDir, RECORD_DIRS, TASKS_DIR, TRACKER_LABEL,
} from '../schema.js';
...
export async function initStore (options) {
  ...
  await mkdir(join(store, TASKS_DIR), { recursive: true });
  // `docs/` is deliberately NOT created — see `RECORD_DIRS`. A store grows one when a `doc`
  // record is first written; readers treat its absence as ordinary.
  await mkdir(join(store, RECORD_DIRS.decision), { recursive: true });
  await writeFile(join(store, TASKS_DIR, `tasks-${slug}.yml`), STARTER, 'utf8');

  return {
    root,
    created: [`${name}/${TASKS_DIR}/tasks-${slug}.yml`, `${name}/${RECORD_DIRS.decision}/`],
  };
}
