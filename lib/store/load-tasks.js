/**
 * load-tasks.js — reading the flat-YAML task store into typed rows.
 *
 * This module takes a RESOLVED root. Finding the store — and being the only place that can say
 * it failed to — is `store/root.js`, which is where a store-not-found throw belongs. (The
 * charter below sat here while this file was a monolith and still reads as though it owned
 * that job; relocating it properly is its own change, but the claim is corrected here so the
 * file does not advertise a contract it cannot keep.)
 *
...
import {
  ID_RE, isNil, isPriority, isStatus, isTaskType, nsId,
} from '../schema.js';
 ...
 * half-globalized `parent` — see schema.js's `GlobalId`. The runtime half is `ids.js`:
 * `nsId` mints from anything, so the value has to be judged BEFORE it is minted from.
 *
 * @typedef Task
...
/**
 * The `deps` list — the array wholesale, then every element on its own.
 *
 * PER ELEMENT, where `labels` and `acceptance_criteria` reject their whole list. The asymmetry
 * is deliberate: those are attributes, and a half-read set of labels is a worse answer than
 * none, but a dep is an EDGE and each one stands alone — dropping the good edges because a
 * sibling is malformed would blind the ready computation to real blockers. `bd-map.js` already
 * rejects bd's dependency edges one at a time, for the same reason.
 *
 * A non-array `deps:` is still wholesale, because there are no elements to salvage.
 *
 ...
 * @returns {GlobalId[]}
 */
function safeDeps (rawDeps, slug, reject) {
  if (isNil(rawDeps)) return [];

  if (!typesafeIsArray(rawDeps)) {
  ...
  for (const dep of rawDeps) {
    if (isUsableRef(dep)) deps.push(nsId(dep, slug));
    else reject('deps', dep, 'this dependency is DROPPED — the row can be offered as READY while a real blocker is still open, and the `(missing)` line that would have named the broken edge is gone from `ready --json` with it');
  }
...
function createTaskFromRawData (id, slug, raw, reject) {
  ...
  const task = {
    id,
    deps: safeDeps(raw['deps'], slug, reject),
  };
  ...
  // THE ONE FIELD HERE THAT LAUNDERED RATHER THAN DROPPED. Its siblings below discard a value
  // of the wrong shape; this one accepted EVERY non-nil value and passed it through `nsId`,
  // minting a `GlobalId` out of whatever was written. An unquoted `parent: 42` is a YAML
  // number, and `backlog/42` is a name another row may already answer to — so the typo did not
  // lose an edge, it FABRICATED one, and made an untouched row the container of a child it
  // never had.
  if (isUsableRef(raw['parent'])) task.parent = nsId(raw['parent'], slug);
  else if (!isNil(raw['parent'])) reject('parent', raw['parent'], 'the row is treated as having NO parent, so its container is no longer blocked by it and can be offered as ready work while this row is still open');
...
export async function loadTasks (root, warn = () => {}) {
  ...
  for await (const { file, raw, slug } of loadRawTasks(tasksDir, names, warn)) {
    ...
    // SKIPPED, WHERE A BAD `status` OR `type` IS KEPT — and the difference is not severity, it
    // is that `id` is the KEY. An attribute may honestly be `undefined`, which is what the
    // status comment below argues and why it refuses a sentinel; a key may not. The only key
    // available for a row whose id is unusable is `String(raw['id'])`, which is exactly the
    // laundering this guard exists to remove — so "keep it" cannot be implemented without
    // re-introducing the defect. There is no honest carrier for a missing key.
    //
    // "Represent the malformed input; never delete it" (above) is satisfied here the same way
    // it is for an unparseable FILE: by the warning — which becomes `unsound`, and exits 2
    // under `--strict` — and not by inventing a name for the payload.
    if (!isUsableId(raw['id'])) {
      warn(`${file}: a task entry has an unusable id ${renderRejected(raw['id'])} — an id must be a string matching ${ID_RE.source}, so the row is SKIPPED and is MISSING from every count and every partition (run \`diarie validate\`)`);
      continue;
    }
