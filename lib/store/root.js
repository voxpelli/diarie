...
function assertNotPluginsOwnStore (dir) {
  ...
  // ITS OWN CODE, not an `ENOSTORE` with a nicer message. `cli.js` serialises `{error, code,
  // body}`, so under `--json` a caller branches on the code and nothing else — and the
  // documented answer to `ENOSTORE` is `diarie init`, which HERE would create a store inside
  // the plugin cache. A refusal whose remedy makes things worse has to be distinguishable by
  // the field consumers actually read.
