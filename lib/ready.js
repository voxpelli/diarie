 ...
 *
 * Pure computation over a loaded task list. Finding and loading the store belongs to the
 * `store/` modules — `store/root.js` resolves, `store/load-tasks.js` reads. This file never
...
/** @import { GlobalId, Status} from './schema.js' */
/** @import { Task } from './store/load-tasks.js' */
 ...
 *
 * @template {string} T
 * @param {ReadonlySet<T> | Set<T>} keys
 ...
 *
 * @template {string} T
 * @param {Record<T, number>} counts
 * @param {T | undefined} key
 * @returns {void}
 */
...
/**
 * What a row's status reads as in a diagnostic when it does not have one.
 *
 * `status` is optional on `Task` because the loader omits it when the YAML said something the
 * schema does not know. Every blocker line that names a status therefore has to render a case
 * that is not a `Status` — and rendering it as the empty string, or as the word "undefined",
 * would produce `backlog/T-1 ()`, a line whose parenthesis is the only evidence anything is
 * wrong.
 *
 * A separate word rather than a member of `Status`: the vocabulary is what consumers switch on
 * and what `--filter` accepts, so this must never be answerable there. It is display only.
 */
const NO_STATUS = 'invalid status';

/**
 * @param {Status|undefined} status
 * @returns {Status | typeof NO_STATUS}
 */
const statusLabel = (status) => status ?? NO_STATUS;
 ...
 * live in the store's `decisions/`, outside `loadTasks`'s glob) — but `milestone` is NOT:
 ...
 *
 * @param {Task[]} tasks
 * @returns {Map<GlobalId, { active: GlobalId[], stalled: `${GlobalId} (${Status | typeof NO_STATUS})`[] }>}
 */
function childrenByParent (tasks) {
  /** @type {Map<GlobalId, { active: GlobalId[], stalled: `${GlobalId} (${Status | typeof NO_STATUS})`[] }>} */
  const kids = new Map();
  ...
  for (const t of tasks) {
    ...
    if (!entry) {
      entry = { active: [], stalled: [] };
      kids.set(t.parent, entry);
    }

    if (t.status === 'pending' || t.status === 'in_progress') {
      entry.active.push(t.id);
    } else {
      entry.stalled.push(`${t.id} (${statusLabel(t.status)})`);
    }
  }
...
export function computeReady (tasks) {
  ...
  /** @type {Task[]} */
  const ready = [];
  /** @type {Array<Task & { blockers: GlobalId[], children?: GlobalId[], attention?: string[] }>} */
  const blocked = [];
  /** @type {Array<Task & { reason: string }>} */
  const needsAttention = [];

  for (const task of tasks) {
    // A row with NO USABLE STATUS is malformed, and it must be caught HERE — before the
    // `!== 'pending'` filter below, which would otherwise discard it as though it were an
    // ordinary completed row, leaving it in no partition while `total` still counts it.
    // Present in the sum, absent from every answer: the same shape as the `type` case below.
    //
    // GATED ON THE TYPE, so the rule that non-task types never enter needsAttention survives:
    // a `decision` row with a broken status is still excluded in silence, because a decision
    // is not work whatever its status says. `undefined` type is included deliberately — we
    // cannot tell it is not work, and the `type` branch below would never see it.
    if (task.status === undefined) {
      if (task.type === 'task' || task.type === undefined) {
        needsAttention.push({
          ...task,
          reason: 'missing or invalid `status` — it belongs in no partition and no tally (run `diarie validate`)',
        });
      }
      continue;
    }
    ...
    if (task.type === undefined) {
      needsAttention.push({
        ...task,
        reason: 'missing or invalid `type` — it belongs in no partition (run `diarie validate`)',
      });
      continue;
    }
    ...
    /** @type {(`${GlobalId} (${Status | typeof NO_STATUS | 'missing'})` | `parent ${GlobalId} (missing)`)[]} */
    const stalled = []; // missing, or terminal-but-not-completed

    for (const depId of task.deps ?? []) {
      const dep = byId.get(depId);

      if (!dep) {
        stalled.push(`${depId} (missing)`);
      } else if (dep.status === 'completed') {
        continue;
      } else if (dep.status === 'pending' || dep.status === 'in_progress') {
        active.push(depId);
      } else {
        stalled.push(`${depId} (${statusLabel(dep.status)})`);
      }
    }
    ...
    if (task.parent && !byId.has(task.parent)) {
      stalled.push(`parent ${task.parent} (missing)`);
    }
