 ...
 *
 * @param {Task[]} tasks
 * @returns {Map<GlobalId, { active: GlobalId[], stalled: `${GlobalId} (${Status})`[] }>}
 */
function childrenByParent (tasks) {
  /** @type {Map<GlobalId, { active: GlobalId[], stalled: `${GlobalId} (${Status})`[] }>} */
  const kids = new Map();
  ...
  for (const t of tasks) {
    ...
    if (t.status === 'pending' || t.status === 'in_progress') {
      ...
      entry.stalled.push(`${t.id} (${t.status})`);
    }
  }
...
export function computeReady (tasks) {
  ...
  for (const task of tasks) {
    ...
    /** @type {(`${GlobalId} (${Status | 'missing'})` | `parent ${GlobalId} (missing)`)[]} */
