 ...
 * @returns {v is string}
 */
export const isUsableBdId = (v) => isType(v, 'string') && ID_RE.test(v);
 ...
 *
 * NEITHER guards the VALUE at runtime. `hasOwn({...TYPE_MAP, bug: {type: 'bugg'}},
 * 'bug')` is `true` and `'bugg'` reaches the written row unchecked — that is exactly
 * what the deleted `VALID_TYPES.has(mapped.type)` was for, and restoring it in
 * `projectLive` (the path that WRITES a store) is tracked as its own change.
 *
...
export function censusFields (records) {
  ...
  for (const r of records) {
    for (const [key, value] of Object.entries(r)) {
      ...
      if (guardedArrayIncludes(CONSUMED_BD_FIELDS, key)) {
        continue;
