 ...
 * @property {string} [notes]         free prose; no schema field, folded into `description`
 * @property {string} [design]        free prose; same treatment as `notes`
 */
 ...
 */
const CONSUMED_BD_FIELDS = new Set([
...
export function censusFields (records) {
  ...
  for (const r of records) {
    for (const [key, value] of Object.entries(r)) {
      ...
      if (!present || CONSUMED_BD_FIELDS.has(key)) continue;
