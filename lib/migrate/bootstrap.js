...
export function projectLive (r, liveIds, droppedEdges, priorityDefaulted = [], droppedLabels = []) {
  ...
  //
  // The value's only other guard is the `@satisfies` cast on the map, which is compile-time and
  // fails OPEN in the one way that matters: misspell the TAG itself (`@satisifes`) and tsc
  // ignores it silently, where a misspelled TYPE is a hard error. So a bad map edit by anyone
  // who does not run the gate reaches the written store unchecked.
  //
