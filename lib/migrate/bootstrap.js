...
import { isStringArray, isType, typesafeIsArray } from '@voxpelli/typed-utils';
...
import {
  assertStorePathFree, legacyTrackerDirIn, ROOT_ENV_NAMES, trackerDirIn,
} from '../store.js';
import {
 ...
 * the typedef is a hope, not a guarantee. Anything that is not a string yields no section and
 * is left for the residue census to report.
 *
...
export function projectLive (r, liveIds, droppedEdges, priorityDefaulted = [], droppedLabels = []) {
  ...
  // stack trace and an empty stdout. A non-string still surfaces in the residue census.
  const body = [
    description,
