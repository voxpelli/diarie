...
import { loadedTaskToTask, loadTasksWithWarnings } from '../store/load-tasks.js';
...
const flags = /** @satisfies {AnyFlags} */ ({
  ...
  strict: {
    // NAMES ALL THREE CAUSES. It used to say "any task needs attention, or the queue looks
    // cyclic" while `unsound` ALSO fires on `warnings.length > 0` — a loader warning is neither
    // of those. So the flag that exists to make the answer self-certifying was under-describing
    // its own verdict, in `--help`, which is the surface a prose-command checker interrogates.
    // A report that does not name its cause is the defect this flag was added to catch.
    description: 'Exit 2 if the answer cannot be trusted: a field was rejected, a task needs attention, or nothing is ready while work remains',
    type: 'boolean',
    'default': false,
  },
});
...
function setupCommand (name, description, args, meta) {
  const { flags: opts } = peowly({
    ...meta,
    // THE CANONICAL COPY OF THIS COMMENT — `stats`, `validate` and `init` point here.
    //
    // peowly hardcodes `allowPositionals: true` and then spreads the caller's rest options
    // OVER it (peowly.js:160-162), so this is a supported typed pass-through, not a hack:
    // `ExtendedParseArgsConfig` omits only `strict`/`tokens`/`options` from `ParseArgsConfig`.
    // Note it goes at the TOP LEVEL — `parseArgsOptions` is a rest destructure (peowly.js:36),
    // not a property, so nesting it under that name would silently do nothing.
    //
    // Without it a positional is DISCARDED IN SILENCE. `diarie ready pending` — the natural
    // wrong guess for `--filter pending`, and the shape bd's CLI actually accepted — printed
    // the full unfiltered backlog and exited 0. Not a subset, not an error: a confident,
    // plausible, wrong answer, which is this package's founding defect with no store involved.
    //
    // It also makes lib/cli.js honest. That file's ERR_PARSE_ARGS_ branch has always claimed
    // it handles "an unknown flag, a bad flag value, or an unexpected positional"; the third
    // could not fire until now, so the comment became true on the day this line was written.
...
function formatWorkResult (workResult, { blocked, json, parentName, strict }) {
  ...
  if (workResult.mode === 'filter') {
    ...
    if (json) jsonOut(tasks.map(t => loadedTaskToTask(t)));
  ...
  if (json) {
    ...
    jsonOut({
      ready: partition.ready.map(t => loadedTaskToTask(t)),
      blocked: partition.blocked.map(t => loadedTaskToTask(t)),
      needsAttention: partition.needsAttention.map(t => loadedTaskToTask(t)),
