...
import { LEGACY_TRACKER_DIRS, TRACKER_DIRS, TRACKER_LABEL } from '../schema.js';
import { resolveInitRoot, trackerDirIn } from '../store.js';
...
const flags = /** @satisfies {import('peowly').AnyFlags} */ ({
  ...
  dotted: {
    description: `Create ${TRACKER_DIRS[1]}/ instead of ${TRACKER_DIRS[0]}/ (the reading-room posture)`,
    type: 'boolean',
    'default': false,
  },
});
...
export const init = {
  description: `Create a ${TRACKER_DIRS[0]}/ task store`,
 ...
 *
 * Three refusals, in order — see the comments; each answers a different question, and
...
export async function doTheWork ({ dotted, root, slug }) {
  const name = dotted ? TRACKER_DIRS[1] : TRACKER_DIRS[0];
  ...
  const legacy = LEGACY_TRACKER_DIRS.find(dir => existsSync(join(root, dir)));
  if (legacy) {
    throw new InputError(
      `${legacy}/ already exists in ${root} — that IS your store, at the name diarie used before ${TRACKER_LABEL}. ` +
      `Rename it rather than starting a second one: \`git mv ${legacy} ${name}\``,
      undefined,
      'ELEGACY'
    );
  }
