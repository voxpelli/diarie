...
export const REQUIRED_FIELDS = /** @satisfies {RequiredKeysOf<TaskRow>[]} */ (/** @type {const} */ ([
  'id',
  'title',
  'status',
  'type',
 ...
 *   EUSAGE      you typed it wrong (bad command, bad flag, a retired env var still set)
 ...
 *
 * Adding one is a public-contract change: give it a line here, a row in README's exit-code
 * table, and a case in the CLI-boundary test — which fails if a code goes unexercised, so the
 * vocabulary cannot grow a member that nothing proves reachable.
 *
...
export const VALID_ERROR_CODES = /** @type {const} */ ([
  'ENOSTORE', 'EUSAGE', 'EEXIST', 'ETWOSTORES', 'ELEGACY', 'ELOSSY',
]);
...
/** @typedef {(typeof TRACKER_DIRS)[number]} TRACKER_DIR_TYPES */
...
/** @typedef {(typeof LEGACY_TRACKER_DIRS)[number]} LEGACY_TRACKER_DIR_TYPES */
 ...
 *
 * @param {unknown} v
 * @returns {v is TRACKER_DIR_TYPES[number]}
 */
 ...
 *
 * @param {unknown} v
 * @returns {v is TRACKER_DIR_TYPES | LEGACY_TRACKER_DIR_TYPES}
 */
