...
store is a _state, not a typo_, which is exactly `init.js`'s own stated reason for giving
...
1. **Ancestor `.diarie/` exists, the target dir is fresh** → refuse unless `--nested`. The
   question is _"did you mean to shadow the store above you for this subtree?"_
2. **A `.diarie/` already exists AT the target** → refuse for content-safety; `--force`
   overrides. The question is _"may I overwrite the store that is already here?"_ This is
...
is about _this_ directory, the most specific fact on disk. The ancestor check (case 1,
...
nested stores are independent, nearest-wins, and benign — so the class norm is _silent nesting_
...
forgotten-ancestor nesting. `ENOSTORE` only distinguishes present-vs-absent, and _which_ of two
present stores answers is already unambiguous via nearest-wins + a `diarie where` query — so the
refuse does not buy _determinism_ (nearest-wins gives that). What it buys is _accident-prevention_:
...
**`DIARIE_ROOT`, not `DIARIE_DIR`.** The value is a _project root_ — the directory that CONTAINS
...
* **Plugin/symlink guard on the new ancestor-walk — make it STRUCTURAL, not a "MUST" (implementer
  ...
  less). But do NOT satisfy this with a hand-rolled second walk that _remembers_ to call the
...
* 2026-07-28 — **the env rename shipped, as `DIARIUM_ROOT` rather than `DIARIE_ROOT`.** This
  ...
  `_DIR`, because the value is the _project root that holds_ the register, never the register
