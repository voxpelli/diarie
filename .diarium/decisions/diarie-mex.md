...
read-only _spike_ ("its one-time job being done", "ships untested by design"). But its three maps are
NOT orphaned — the _shipped_ migrator behind `diarie migrate`, `bootstrap.js`, imports them as the
single source of truth. Only `projectRecords` is the `@planned` / knip-flagged export with no caller.
So exposing the _file_ wholesale would publish the disposable projector and its CLI as semver-committed
...
The runtime library should ship the migration _mechanism_; a specific competitor's _vocabulary_ should
not enter its permanent API. Against diarie's tenets, a subpath export is the only option that
_violates_ two of them: lock-in resistance (a semver-frozen entrypoint carrying a dead competitor's
...
* **Rails / ActiveRecord** — migrations live in the app, not in the ORM's public API; the ORM ships the
  migration _engine_, not any app's migration vocabulary.
...
base"; Michael Nygard's original format makes supersession an _authored_ pointer with an explicit
...
* **Provider-canonical, with a consumer stub.** This record is canonical: diarie owns `exports`, so it
  is the upstream that _defines_ the contract; vp-skills' `vp-beads-dad` is the consumer side and should
...
* **Pressures `diarie-rel`.** This is the first real case that forces the relation-model question to
  decide whether a relation's target may be a _foreign-store_ id (`vp-beads-dad` lives in another
