...
* **Data quality over lossy prose.** A file path or a cross-reference buried in a title/description
  is unqueryable. As structured fields they support `diarie list --file <path>` and `--rel <relation>` (task `diarie-lst`) and, for `files`, the file-contention partitioning diarie's own
...
The motivating example used `task: diarie-reg`, but `diarie-reg` is a _decision_ — so the
...
* **A closed, diarie-defined relation enum for `links`** — declined: it violates the open-relation
  ...
  ever making a relation _computed_ — see `diarie-rel`.)
