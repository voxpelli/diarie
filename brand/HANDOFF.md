...
README and the OFL license texts — the OFL asks that they travel with the font
software), and `CNAME` + `.nojekyll`. Source `brand/` stays pristine — its
...
diarie.dev custom domain (the `CNAME` in the artifact keeps it pinned).

`brand-book.html` is NOT deployed — it works from `file://` by design (keep
`fonts/` beside it), a local design reference only.
