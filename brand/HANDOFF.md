...
self-hosted woff2:s from the adjacent `fonts/` directory — the same
directory `update-stamp.mjs` reads; there is no build step to run in
production — the reference
...
| `.dnr` in `.dnr-scene` | INKOM date stamp, **outlined SVG**          | Rotation is baked into the geometry — immune to per-glyph pixel snapping (Bugzilla 492214). Colors route via `--st-line`/`--st-text`. Two complete rings behind, sized to fit — the no-clipped-arcs rule applies here too. Below 840px the scene centers on its own wrapped row (`margin: auto`) instead of hugging the left edge. Date and version are regenerated by `update-stamp.mjs` at release                                                                                              |
...
| Tier                                                 | Mechanism                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Baseline (iPhone 12 mini, 375px; any browser; no JS) | Fluid `clamp()` type; no fixed heights; the install pill compacts below 540px and fits one line down to 375px, wrapping only as a safety net (translations, zoom); terminals scroll rather than break; tap targets ≥ 32px (nav padding, copy button); `viewport-fit=cover` + `env(safe-area-inset-*)` on body and footer; `text-size-adjust: 100%`                                                                                                                                                                                                                                                                                    |
...
diarie.dev root = `index.html` + `og.png` + `apple-touch-icon.png` +
the `fonts/` directory (three woff2:s, plus README and the OFL license
texts — the OFL asks that they travel with the font software). Everything else is repo
material. `brand-book.html` works from `file://` by design (keep `fonts/` beside
it). At first npm
